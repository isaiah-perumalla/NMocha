...
namespace NMock2.Internal {
    ...
    public class CompositeType : IEquatable<CompositeType> {
        ...
        public CompositeType(Type type, Type[] additionalTypes) {
            ...
            var combinedTypes = new Type[additionalTypes.Length + 1];
        ...
        public bool Equals(CompositeType other) {
            ...
            if (containedTypes.Length != other.containedTypes.Length)
            {
                return false;
            }
            ...
            {
                if (!containedTypes[i].Equals(other.containedTypes[i]))
                {
                    return false;
                }
            }
        ...
        public IList<MethodInfo> GetMatchingMethods(Matcher matcher, bool firstMatchOnly) {
            ...
            foreach (Type type in containedTypes)
            {
                ...
                else
                {
                    ...
                    {
                        if (IsMethodVisible(method) && matcher.Matches(method))
                        {
                            matches.Add(method);
        ...
        public override string ToString() {
            ...
            for (int i = 0; i < containedTypes.Length; i++)
            {
                ...
                sb.Append(containedTypes[i].Name);
            }
        ...
        private void Initialize(Type[] types) {
            ...
            containedTypes = RationalizeTypes(CreateOrderedCopyOfTypes(types));

            if (containedTypes.Length > 1)
            {
                additionalTypes = new Type[containedTypes.Length - 1];
                Array.Copy(containedTypes, 1, additionalTypes, 0, additionalTypes.Length);
            }
        }

        private Type[] CreateOrderedCopyOfTypes(Type[] types) {
            var orderedTypes = types.Clone() as Type[];

            Array.Sort(orderedTypes,
                       (t1, t2) => string.CompareOrdinal(t1.AssemblyQualifiedName, t2.AssemblyQualifiedName));
        ...
        private Type[] RationalizeTypes(Type[] types) {
            ...
            foreach (Type type in types)
            {
                if (!type.IsInterface)
                {
                    if (type.IsClass)
                    {
                        ...
                        if (baseClass != null)
                        {
                            throw new ArgumentException(
                                "Cannot mock more than one class type in a single mock instance.", "types");
                        }
                    ...
                    {
                        throw new ArgumentException(
                            "Can only mock class and interface types. Invalid type was: " + type.Name, "types");
                    }
                }
                else
                {
