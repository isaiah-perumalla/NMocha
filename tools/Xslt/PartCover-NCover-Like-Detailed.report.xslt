...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:user="http://customfunctions">
        ...
        <table class="coverageReportTable" cellpadding="2" cellspacing="0">
          <tbody>
            ...
            <xsl:variable name="threshold">
              <xsl:choose>
                <xsl:when test="$Acceptable > 0">
                  <xsl:value-of select="$Acceptable" />
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of select="//PartCoverExtension/Acceptable" />
                </xsl:otherwise>
              </xsl:choose>
            </xsl:variable>
        ...
        <table width="100%">
          <tbody>
            <tr>
              <td valign="top">
                <h1 class="titleText">
                  PartCover Coverage Report - <xsl:value-of select="$ProjectName" />&#160;&#160;
                </h1>
  ...
  <xsl:template name="classModuleSummary">
    ...
    <xsl:for-each select="$unique-asms">
      ...
      <tr>
        ...
        <td class="secondaryTable mainTableHeader">
          <xsl:value-of select="$unvisitedTitle" />
        </td>
        <td class="secondaryTable mainTableGraphHeader" colspan="2">
          <xsl:value-of select="$coverageTitle" />
        </td>
      </tr>
      <xsl:variable name="current-asm" select="./@asm"/>
      ...
      <xsl:variable name="classes" select="//PartCoverReport/type[@asm=$current-asm]"/>
      <!--<xsl:variable name="namespaces" select="$classes[not(user:GetNamespace(@name)=user:GetNamespace(following-sibling::type/@name))]"/>-->
      <xsl:for-each select="$classes">
        <xsl:sort select="./@name"/>
        <xsl:variable name="current-namespace" select="user:GetNamespace(./@name)"/>
        <xsl:if test="generate-id(.)=generate-id($classes[user:GetNamespace(@name)=$current-namespace])">
          <xsl:variable name="codeSizeNs" select="sum($classes[user:GetNamespace(@name)=$current-namespace]/method/code/pt/@len)+0"/>
        <xsl:variable name="coveredCodeSizeNs" select="sum($classes[user:GetNamespace(@name)=$current-namespace]/method/code/pt[@visit>0]/@len)+0"/>
        <xsl:variable name="coverageNs" select="ceiling(100 * $coveredCodeSizeNs div $codeSizeNs)"/>
        <xsl:call-template name="coverageIndentedDetail">
          <xsl:with-param name="name" select="user:GetNamespace(@name)" />
          <xsl:with-param name="unvisitedPoints" select="($codeSizeNs - $coveredCodeSizeNs)" />
          <xsl:with-param name="sequencePoints" select="$codeSizeNs" />
          <xsl:with-param name="coverage" select="$coverageNs" />
          <xsl:with-param name="threshold" select="$threshold" />
          <xsl:with-param name="styleTweak">padding-left:20px;font-weight:bold</xsl:with-param>
        </xsl:call-template>
        <xsl:for-each select="$classes[user:GetNamespace(@name)=$current-namespace]">
          <xsl:sort select="./@name"/>
          <xsl:variable name="codeSizeClass" select="sum(./method/code/pt/@len)+0"/>
          <xsl:variable name="coveredCodeSizeClass" select="sum(./method/code/pt[@visit>0]/@len)+0"/>
          <xsl:variable name="coverageClass" select="ceiling(100 * $coveredCodeSizeClass div $codeSizeClass)"/>
          <xsl:call-template name="coverageIndentedDetail">
            <xsl:with-param name="name" select="user:GetName(./@name)" />
            <xsl:with-param name="unvisitedPoints" select="($codeSizeClass - $coveredCodeSizeClass)" />
            <xsl:with-param name="sequencePoints" select="$codeSizeClass" />
            <xsl:with-param name="coverage" select="$coverageClass" />
            <xsl:with-param name="threshold" select="$threshold" />
  ...
  <xsl:template name="coverageDetailSecondary">
    ...
    <tr>
      ...
      <td class="mainTableCellData">
        <xsl:value-of select="concat(format-number($threshold,'#0.0'), ' %')" />
      </td>
      <td class="mainTableCellData">
        <xsl:value-of select="$unvisitedPoints" />
      </td>
      <td class="mainTableCellPercent">
        <xsl:value-of select="concat(format-number($coverage,'#0.0'), ' %')" />
      </td>
  ...
  <xsl:template name="coverageIndentedDetail">
    ...
    <tr>
      <td class="mainTableCellBottom childTableCellIndentedItem" colspan="2">
        <xsl:attribute name="style">
          <xsl:value-of select="$styleTweak"/>
        </xsl:attribute>
        <xsl:value-of select="$name" />
      </td>
      <td class="mainTableCellBottom mainTableCellData">
